...
namespace CalamityLegendsComeBack.Weapons.BlossomFlux
{
    internal sealed partial class NewLegendBlossomFluxHoldOut
    {
        ...
        public static string CoreStarTexture = "CalamityMod/Particles/HalfStar";

        // 旋转速度基数
        public static float CoreRotationSpeedMultiplier = 1.0f;

        // 核心星芒大小/缩放比例
        public static float CoreStarScaleX = 0.35f;  // 宽度（越大越宽/圆润，越小越尖锐，SHPC默认0.3左右）
        public static float CoreStarScaleY = 1.0f;   // 长度系数
        public static float CoreBloomScaleX = 0.45f; // 背景光晕宽度
        public static float CoreBloomScaleY = 0.25f; // 背景光晕高度
        // =============================================================
        ...
        private void DrawSHPCMagicCore(BlossomFluxChloroplastPresetType preset, float power, float phaseKick, bool rightCharge, bool idleCore)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Main.EntitySpriteDraw(
                    ...
                    core,
                    null,
                    Color.Lerp(cyan, white, charge) * bloomOpacity * (1f - 0.2f * i),
                    time * 0.5f * (i % 2 == 0 ? 1f : -1f),
                    bloom.Size() * 0.5f,
                    bloomScale,
                    SpriteEffects.None,
                    0);
            ...
            // 绘制星芒层级，采用 SHPC 的 5 层重叠动态缩放与交叉旋转算法
            int layers = 5;
            for (int i = 0; i < layers; i++)
            {
                ...
                for (int b = -1; b <= 1; b += 2)
                {
                    ...
                    float starOpacity = idleCore
                        ? 0.28f + charge * 0.12f
                        : 0.42f + charge * 0.28f + flashPulse * 0.45f;
                    ...
                    Main.EntitySpriteDraw(
                        ...
                        core,
                        null,
                        Color.Lerp(cyan, white, 0.58f) * starOpacity * (1f - 0.1f * i),
                        rotation,
                        star.Size() * 0.5f,
                        starScale,
                        SpriteEffects.None,
