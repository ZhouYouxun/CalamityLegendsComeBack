...
namespace CalamityLegendsComeBack.Weapons.PristineFury
{
    internal sealed class PristineFuryHook : ModProjectile, ILocalizedModType
    {
        ...
        private void SpawnHitBurst(Color color)
        {
            ...
            for (int i = 0; i < 24; i++)
            {
                Dust dust = Dust.NewDustPerfect(Projectile.Center, DustID.Electric, Main.rand.NextVector2CircularEdge(1f, 1f) * Main.rand.NextFloat(2f, 7f), 80, color, Main.rand.NextFloat(0.8f, 1.4f));
                dust.noGravity = true;
            }
