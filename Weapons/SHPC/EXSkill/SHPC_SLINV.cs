...
namespace CalamityLegendsComeBack.Weapons.SHPC.EXSkill
{
    ...
    internal class SHPC_ChargeSpiralInv : ModProjectile, ILocalizedModType
    {
        ...
        private const float FinalRadius = 8f;
        ...
        private float InitialRadius => MathHelper.Max(16f, Projectile.ai[2]);
        ...
        {
            initialized = false;
            angularVelocity = Main.rand.NextFloat(0.13f, 0.2f) * (Main.rand.NextBool() ? 1f : -1f);
        ...
        public override void AI()
        {
            ...
            float shrink = (float)System.Math.Pow(1f - progress, 2.35f);
            float radius = MathHelper.Lerp(FinalRadius, InitialRadius, shrink);
            Vector2 axis = ownerProj.velocity.SafeNormalize(Vector2.UnitX);
            Vector2 origin = ownerProj.Center + axis * 56f;

            OrbitAngle += angularVelocity * MathHelper.Lerp(1.25f, 2.25f, progress);
            Vector2 nextCenter = origin + OrbitAngle.ToRotationVector2() * radius;
            ...
            if (progress > 0.7f && radius <= FinalRadius + 3f)
                Projectile.Kill();
        }
