...
namespace CalamityLegendsComeBack.Weapons.YharimsCrystal.LeftGeneral
{
    internal sealed class YC_EssenceFlame : ModProjectile, ILocalizedModType, IPixelatedPrimitiveRenderer
    {
        ...
        private const int MaxOwnedEssenceFlames = 5;
        private const int OrbitSlots = 5;
        private const int MaxSplitGeneration = 1;
        private const float SplitGenerationPhaseStride = 10000f;

        private static readonly Color CoreGold = new(255, 214, 88);
        private static readonly Color CoreRed = new(255, 111, 34);

        private ref float TargetIndex => ref Projectile.ai[0];
        private ref float Phase => ref Projectile.ai[1];
        private ref float Timer => ref Projectile.localAI[0];
        private float VisualPhase => Phase % SplitGenerationPhaseStride;
        private int SplitGeneration => Math.Max(0, (int)MathF.Floor(Phase / SplitGenerationPhaseStride));
        private int LaneIndex => PositiveModulo((int)MathF.Floor(VisualPhase), OrbitSlots);
        private int FlameProfile => PositiveModulo((int)MathF.Floor(VisualPhase / OrbitSlots), 4);
        private float CenteredLane => LaneIndex - (OrbitSlots - 1) * 0.5f;
        private float Seed => VisualPhase * 0.613f + Projectile.identity * 0.173f;
        private float ReleaseDelay => 36f + LaneIndex * 3f + FlameProfile * 1.75f;
        ...
        internal static bool CanSpawnMoreFor(Player player, int queued = 0)
        {
            return CountOwnedEssenceFlames(player, queued) < MaxOwnedEssenceFlames;
        }

        internal static float NextOrbitPhaseFor(Player player)
        {
            int flameType = ModContent.ProjectileType<YC_EssenceFlame>();
            bool[] usedSlots = new bool[OrbitSlots];

            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                Projectile projectile = Main.projectile[i];
                if (projectile.active && projectile.owner == player.whoAmI && projectile.type == flameType)
                {
                    int slot = PositiveModulo((int)MathF.Floor(projectile.ai[1] % SplitGenerationPhaseStride), OrbitSlots);
                    usedSlots[slot] = true;
                }
            }
        ...
        internal static float GetOrbitAngle(float phase, float timer, int direction)
        {
            float visualPhase = phase % SplitGenerationPhaseStride;
            int lane = PositiveModulo((int)MathF.Floor(visualPhase), OrbitSlots);
            float spinDirection = direction >= 0 ? 1f : -1f;
            return MathHelper.TwoPi * lane / OrbitSlots + timer * 0.045f * spinDirection;
        }

        private static int CountOwnedEssenceFlames(Player player, int queued = 0)
        {
            int flameType = ModContent.ProjectileType<YC_EssenceFlame>();
        ...
        public override void AI()
        {
            ...
            if (Timer < ReleaseDelay || target is null)
            {
                OrbitOwner(currentDirection);
                Projectile.rotation = Projectile.velocity.SafeNormalize(Vector2.UnitX).ToRotation() + MathHelper.PiOver2;
                Lighting.AddLight(Projectile.Center, Color.Lerp(CoreRed, CoreGold, 0.5f).ToVector3() * 0.8f * Projectile.Opacity);
                SpawnFlightVFX();
                return;
            }
        ...
        private void OrbitOwner(Vector2 fallbackDirection)
        {
            Player owner = Main.player[Projectile.owner];
            if (!owner.active || owner.dead)
                return;

            float orbitAngle = GetOrbitAngle(Phase, Timer, owner.direction);
            ...
            float radius = 132f + Math.Abs(CenteredLane) * 8f + FlameProfile * 5f;
            Vector2 desiredCenter = owner.MountedCenter + orbitDirection * radius;
            Vector2 toDesired = desiredCenter - Projectile.Center;
            Vector2 desiredVelocity = toDesired * 0.13f + tangent * 3.2f;

            if (Timer < 4f || toDesired.Length() > 520f)
        ...
        private void SpawnFlightVFX()
        {
            ...
            Color mainColor = EnergyColor(Timer * 0.025f + VisualPhase * 0.07f);

            Dust dust = Dust.NewDustPerfect(
                Projectile.Center - direction * 10f + normal * Main.rand.NextFloat(-4f, 4f),
                Main.rand.NextBool() ? DustID.GoldFlame : DustID.CopperCoin,
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (!Main.dedServ)
            {
                for (int i = 0; i < 8; i++)
                {
                    Vector2 velocity = Main.rand.NextVector2Circular(4f, 4f) + Projectile.velocity * 0.2f;
                    Dust.NewDustPerfect(target.Center, DustID.GoldFlame, velocity, 100, default, Main.rand.NextFloat(0.8f, 1.2f)).noGravity = true;
                }
            }
            ...
            Player owner = Main.player[Projectile.owner];
            if (!CanSpawnMoreFor(owner))
                return;

            for (int i = 0; i < 3; i++)
            {
                if (!CanSpawnMoreFor(owner, i))
                    break;
                ...
                int child = Projectile.NewProjectile(
                    ...
                    Projectile.owner,
                    -1f,
                    VisualPhase + 10f + (SplitGeneration + 1) * SplitGenerationPhaseStride);
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 direction = Projectile.velocity.SafeNormalize(Vector2.UnitX);
            Color color = EnergyColor(Timer * 0.018f + VisualPhase);
            Color additiveColor = color;
            additiveColor.A = 0;
            float fade = Projectile.Opacity;
            float pulse = 0.88f + 0.12f * MathF.Sin(Main.GlobalTimeWrappedHourly * 14f + VisualPhase);
            ...
            Main.EntitySpriteDraw(
                ...
                null,
                Color.White with { A = 0 } * 0.5f * fade,
                Projectile.rotation,
                bloom.Size() * 0.5f,
        ...
        {
            Color head = Color.Lerp(Color.White, EnergyColor(VisualPhase + completion * 0.25f), Utils.GetLerpValue(0f, 0.22f, completion, true) * 0.55f);
            Color tail = Color.Lerp(new Color(64, 28, 8), Color.Transparent, Utils.GetLerpValue(0.58f, 1f, completion, true));
            head.A = 0;
            tail.A = 0;
        ...
        {
            Color color = Color.Lerp(Color.White, EnergyColor(VisualPhase + completion * 0.35f), 0.34f);
            color.A = 0;
        ...
        {
            float t = (float)Math.Sin(offset * 2.5f + Projectile.identity * 0.11f) * 0.5f + 0.5f;
            Color color = Color.Lerp(CoreRed, CoreGold, t);
            color.A = 0;
            return color;
        }
    }
