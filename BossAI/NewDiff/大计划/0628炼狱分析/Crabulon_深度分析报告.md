...
============================================================
本报告针对 Terraria 模组 **Infernum Mode** 中的 Boss `Crabulon` 进行底层的源码级行为与视觉特效分析。分析内容包含其行为重写类、攻击状态机设计、专属弹幕代码结构、核心 AI 算法、Shaders 着色器渲染机制以及屏幕特效控制。

------------------------------------------------------------

## 一、Boss 文件结构与基础信息 (File Structure & Base Info)
- **内部标识名 (Boss ID)**: `Crabulon`
- **重写的NPC目标 (Override Target)**: `ModContent.NPCType<CrabShroom>()`, `ModContent.NPCType<CrabulonNPC>()`
- **模组内关联的源文件列表**:
  - `CrabShroomBehaviorOverride.cs` (源路径: `.../BossAIs/Crabulon/CrabShroomBehaviorOverride.cs`)
  - `CrabulonBehaviorOverride.cs` (源路径: `.../BossAIs/Crabulon/CrabulonBehaviorOverride.cs`)
  - `HomingSpore.cs` (源路径: `.../BossAIs/Crabulon/HomingSpore.cs`)
  - `MushBombInfernum.cs` (源路径: `.../BossAIs/Crabulon/MushBombInfernum.cs`)
  - `MushroomPillar.cs` (源路径: `.../BossAIs/Crabulon/MushroomPillar.cs`)
  - `SporeCloud.cs` (源路径: `.../BossAIs/Crabulon/SporeCloud.cs`)
- **重写行为实现类 (Behavior Override Classes)**:
  - 类名: `CrabShroomBehaviorOverride` -> 重写目标: `ModContent.NPCType<CrabShroom>()`
  - 类名: `CrabulonBehaviorOverride` -> 重写目标: `ModContent.NPCType<CrabulonNPC>()`
...
### 🎯 状态机枚举: `CrabulonAttackState`
该 Boss 的攻击序列由一个专用的状态机控制，包含以下行为状态：
1. `SpawnWait` - 对应的行为处理状态。
2. `JumpToTarget` - 对应的行为处理状态。
3. `WalkToTarget` - 对应的行为处理状态。
4. `CreateGroundMushrooms` - 对应的行为处理状态。
5. `ClawSlamMushroomWaves` - 对应的行为处理状态。

### ⚔️ 核心行为控制方法 (Core Behavior Methods)
代码中共解析出 **5** 个以 `DoBehavior_` 命名的核心行为控制函数，这些函数承载了 Boss 每一招的具体动作和弹幕逻辑：

#### 📁 方法名: `DoBehavior_SpawnWait`
- **参数列表**: `(NPC npc, float attackTimer)`
...
- **技术实现原理解析**:
  在执行 `SpawnWait` 动作期间，Boss 首先获取当前选定的玩家作为追踪目标。
  该动作主要负责重定位。Boss 会通过插值方法（例如 `Vector2.Lerp` 配合特定的弹性常数）平滑地移动到玩家的上方或侧面。此阶段一般伴随防御力提升或免伤保护。

#### 📁 方法名: `DoBehavior_JumpToTarget`
- **参数列表**: `(NPC npc, Player target, float attackTimer, bool enraged, ref float jumpCount)`
- **运动与控制逻辑**: 修改速度/物理运动 (Velocity Modification), 插值平滑过渡 (Lerp/SmoothStep Interpolation)
- **播放音效 (Sounds Played)**: `SoundID.Item14`
- **生成弹幕 (Projectiles Created)**: 无直接生成弹幕，或由伴生随从/弹幕生成器间接释放。
- **屏幕震动效应 (Screen Shake)**: ❌ 无显著震屏行为
- **源码注释逻辑提取**:
...
- **技术实现原理解析**:
  在执行 `JumpToTarget` 动作期间，Boss 首先获取当前选定的玩家作为追踪目标。
...
- **参数列表**: `(NPC npc, Player target, float attackTimer, bool enraged)`
- **运动与控制逻辑**: 修改速度/物理运动 (Velocity Modification), 插值平滑过渡 (Lerp/SmoothStep Interpolation)
- **播放音效 (Sounds Played)**: 未检测到显式调用 `PlaySound`。
- **生成弹幕 (Projectiles Created)**: 无直接生成弹幕，或由伴生随从/弹幕生成器间接释放。
- **屏幕震动效应 (Screen Shake)**: ❌ 无显著震屏行为
...
#### 📁 方法名: `DoBehavior_CreateGroundMushrooms`
...
- **技术实现原理解析**:
  在执行 `CreateGroundMushrooms` 动作期间，Boss 首先获取当前选定的玩家作为追踪目标。
  该方法使用特定的行为状态控制机制，通过累加局部/全局计时器（如 `attackTimer`）来触发攻击流程。伴随多段条件分支判断（如 `if (attackTimer > threshold)`），Boss 的移动速度、弹幕发射角度会根据战斗持续时间动态增加，极大地提升了玩家在炼狱难度下的生存难度。

#### 📁 方法名: `DoBehavior_ClawSlamMushroomWaves`
- **参数列表**: `(NPC npc, Player target, ref float attackTimer, ref bool usingClaws, bool enraged)`
- **运动与控制逻辑**: 修改速度/物理运动 (Velocity Modification), 插值平滑过渡 (Lerp/SmoothStep Interpolation)
- **播放音效 (Sounds Played)**: 未检测到显式调用 `PlaySound`。
- **生成弹幕 (Projectiles Created)**: 无直接生成弹幕，或由伴生随从/弹幕生成器间接释放。
- **屏幕震动效应 (Screen Shake)**: ❌ 无显著震屏行为
...
- **技术实现原理解析**:
  在执行 `ClawSlamMushroomWaves` 动作期间，Boss 首先获取当前选定的玩家作为追踪目标。
  该方法使用特定的行为状态控制机制，通过累加局部/全局计时器（如 `attackTimer`）来触发攻击流程。伴随多段条件分支判断（如 `if (attackTimer > threshold)`），Boss 的移动速度、弹幕发射角度会根据战斗持续时间动态增加，极大地提升了玩家在炼狱难度下的生存难度。
...
- **运动与碰撞逻辑**: `自定义 AI 逻辑`
- **代码级渲染实现分析**:
  `MushroomPillar` 使用传统的 SpriteBatch 进行绘制，可能通过修改 `SpriteBatch.Begin` 的混合状态（如 `BlendState.Additive` 加法混合）来增强其在黑暗环境下的发光感。此外，弹幕还重写了 `Draw` 逻辑以支持根据其旋转角度（`projectile.rotation`）和速度方向自动偏转贴图。
### 🌀 弹幕类: `HomingSpore`
- **渲染机制**: `自定义绘制 (PreDraw/PostDraw Override)`
- **运动与碰撞逻辑**: `自定义 AI 逻辑`
- **代码级渲染实现分析**:
  `HomingSpore` 使用传统的 SpriteBatch 进行绘制，可能通过修改 `SpriteBatch.Begin` 的混合状态（如 `BlendState.Additive` 加法混合）来增强其在黑暗环境下的发光感。此外，弹幕还重写了 `Draw` 逻辑以支持根据其旋转角度（`projectile.rotation`）和速度方向自动偏转贴图。
### 🌀 弹幕类: `SporeCloud`
- **渲染机制**: `自定义绘制 (PreDraw/PostDraw Override)`
- **运动与碰撞逻辑**: `自定义 AI 逻辑`
- **代码级渲染实现分析**:
  `SporeCloud` 使用传统的 SpriteBatch 进行绘制，可能通过修改 `SpriteBatch.Begin` 的混合状态（如 `BlendState.Additive` 加法混合）来增强其在黑暗环境下的发光感。此外，弹幕还重写了 `Draw` 逻辑以支持根据其旋转角度（`projectile.rotation`）和速度方向自动偏转贴图。
### 🌀 弹幕类: `MushBombInfernum`
- **渲染机制**: `自定义绘制 (PreDraw/PostDraw Override)`
- **运动与碰撞逻辑**: `自定义 AI 逻辑 (继承或参考原版 AIStyle)`
- **代码级渲染实现分析**:
  `MushBombInfernum` 使用传统的 SpriteBatch 进行绘制，可能通过修改 `SpriteBatch.Begin` 的混合状态（如 `BlendState.Additive` 加法混合）来增强其在黑暗环境下的发光感。此外，弹幕还重写了 `Draw` 逻辑以支持根据其旋转角度（`projectile.rotation`）和速度方向自动偏转贴图。
...
- `code`: `npc.Infernum().OptionalPrimitiveDrawer ??= new PrimitiveTrailCopy(ClawArmWidthFunction, c => ClawArmColorFunction(npc, c`
- `code`: `InfernumEffectsRegistry.WoFTentacleVertexShader.UseColor(new Color(70, 90, 166));`
- `code`: `InfernumEffectsRegistry.WoFTentacleVertexShader.UseSecondaryColor(new Color(113, 255, 233));`
...
- `code`: `npc.Infernum().OptionalPrimitiveDrawer.Draw(new List<Vector2>()`
...
============================================================
通过对 `Crabulon` 的完整源码结构梳理，我们可以看出 Infernum Mode 团队在设计该 Boss 时的核心设计思路：
...
3. **声色合一**：每一段大招都紧密配合了 `5` 个行为方法内的屏幕震动与特定的声效调用，通过视觉形变（Shader）和听觉反馈（SoundID）共同烘托出史诗级的战斗史诗感。
