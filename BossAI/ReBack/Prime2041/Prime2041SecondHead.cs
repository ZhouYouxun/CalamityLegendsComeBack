...
namespace CalamityLegendsComeBack.BossAI.ReBack.Prime2041
{
    public class Prime2041SecondHead : ModNPC
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            if (!Main.dedServ)
            {
                EyeTexture = ModContent.Request<Texture2D>("CalamityLegendsComeBack/BossAI/ReBack/Prime2041/SkeletronPrime2HeadGlow");
            }
        }
        ...
        public override void AI()
        {
            ...
            if (!Main.npc[(int)NPC.ai[0]].active || Main.npc[(int)NPC.ai[0]].type != ModContent.NPCType<Prime2041>())
            {
                NPC.life = 0;
            ...
            // Spawn Spazmatism in Master Mode
            if (spawnSpazmatism)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                    Mechs2041Spawn.SpawnSpazmatism(spazmatismSpawnPlayer, NPC.GetSource_FromAI());
        ...
        public override bool CheckDead()
        {
            ...
            {
                NPC nPC = Main.npc[i];
                if (nPC.active && nPC.type == ModContent.NPCType<Prime2041>() && nPC.life > 0)
                {
                    nPC.life = 0;
                    nPC.HitEffect(new NPC.HitInfo());
                    nPC.checkDead();
        ...
        {
            if (EyeTexture != null && EyeTexture.IsLoaded)
            {
                Texture2D glowTexture = EyeTexture.Value;
                SpriteEffects effects = NPC.spriteDirection == 1 ? SpriteEffects.FlipHorizontally : SpriteEffects.None;
                Vector2 drawOrigin = NPC.frame.Size() / 2f;
                Vector2 drawPos = NPC.Center - screenPos;
                Color eyesColor = new Color(150, 100, 255, NPC.alpha);
