...
{
    internal static class YC_WarshipHelper
    {
        public static bool TryGetHoldout(int owner, int holdoutIndex, out Projectile holdoutProjectile, out YC_WarshipHoldout holdout)
        {
            ...
            if (holdoutIndex >= 0 && holdoutIndex < Main.maxProjectiles)
            {
                ...
                if (indexedHoldout.active &&
                    ...
                    indexedHoldout.type == ModContent.ProjectileType<YC_WarshipHoldout>() &&
                    indexedHoldout.ModProjectile is YC_WarshipHoldout indexedMod)
                {
            ...
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                ...
                if (!other.active || other.owner != owner || other.type != ModContent.ProjectileType<YC_WarshipHoldout>())
                    continue;

                if (other.ModProjectile is YC_WarshipHoldout holdoutMod)
                {
        ...
        public static void EmitDust(Vector2 center, Vector2 motion, Color color, float scale = 1f, int dustType = Terraria.ID.DustID.GoldFlame)
        {
            if (Main.dedServ)
                return;
        ...
        public static bool IsOwnedWarshipType(int projectileType)
        {
            return projectileType == ModContent.ProjectileType<YC_WarshipEscort>() ||
                   projectileType == ModContent.ProjectileType<YC_WarshipCruiser>() ||
                   projectileType == ModContent.ProjectileType<YC_WarshipBattleship>() ||
                   projectileType == ModContent.ProjectileType<YC_WarshipLaserShip>() ||
                   projectileType == ModContent.ProjectileType<YC_WarshipRepairShip>() ||
                   projectileType == ModContent.ProjectileType<YC_WarshipAuricRail>() ||
