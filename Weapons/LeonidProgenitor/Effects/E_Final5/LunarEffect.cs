    ...
    public class LunarEffect : LeonidMetalEffect
    {
        ...
        public override void OnHitNPC(LeonidCometSmall meteor, Player owner, NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                int strike = Projectile.NewProjectile(meteor.Projectile.GetSource_FromThis(), spawnPosition, velocity, ModContent.ProjectileType<LeonidLunarStrike>(), meteor.Projectile.damage / 2, 0f, meteor.Projectile.owner);
