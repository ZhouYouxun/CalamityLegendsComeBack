...
- AI/命中粒子特效：被动触手的可见部分靠粒子，主要是 `CustomSpark("CalamityMod/Particles/BloomCircle")`、`DustID.Shadowflame` 和 `DustID.Torch`。
...
- 绘制函数特效：`PreDraw` 画旧位置 `BloomCircle` 残影、火球本体，并调用 `DrawArcNovaOrb` 叠 Arc Nova 核心。
...
- 绘制函数特效：`PreDraw` 用 `Utils.DrawLaser` 绘制 `CalamityMod/Projectiles/LaserProj` 两层激光，并在起点/终点叠 `BloomCircle`。
...
### PFEvilT2_Flame
- 绘制函数特效：`PreDraw` 使用 `BloomCircle`、`BloomLineSoftEdge`、`HalfStar`、`magic_03`、`magic_04` 画法阵/准星式火焰。
...
- 绘制函数特效：`PreDraw` 分段绘制 `BrimstoneRayMid` 和 `BrimstoneRayEnd`，不是 `Utils.DrawLaser`。
...
- 绘制函数特效：无绘制，`PreDraw` 返回 `false`。
- AI/命中粒子特效：生成 `DirectionalPulseRing`、`CustomPulse("SoftRoundExplosion")`、`CustomPulse("BloomCircle")`、`SparkParticle`、`PointParticle` 和金火 Dust。

### PFHardMode_TotalityFire
- 绘制函数特效：使用 `CalamityUtils.DrawAfterimagesCentered`，整体 `PreDraw` 返回 `false`。
...
### PFAurora_Flame
- 绘制函数特效：`PreDraw` 使用 `BloomCircle` 和 `BloomLineSoftEdge` 画极光式光束。
- AI/命中粒子特效：生成 `SparkParticle` 和 `CustomSpark("BloomLineSoftEdge")`。

### PFAurora_MuzzleOrb
- 绘制函数特效：`PreDraw` 使用 `BloomCircle` 与 `HalfStar` 画枪口球。
...
- 绘制函数特效：无绘制，`PreDraw` 返回 `false`。
- AI/命中粒子特效：没有粒子，主要是鼠标准星/定位控制弹幕。

### PFGoliath_HiveNukeMissile
- 绘制函数特效：`PreDraw` 使用 `CalamityUtils.DrawAfterimagesCentered`，并叠 `StarProj` 和 `FlameExplosion` 光效。
...
### PFMoonlord_Flame
- 绘制函数特效：`PreDraw` 使用 `BloomLineSoftEdge` 画月总主题火线。
- AI/命中粒子特效：生成 `GlowOrbParticle`。
...
- AI/命中粒子特效：沿束生成 `BloomLineVFX`、`GlowOrbParticle`、`GlowSparkParticle`、`SparkParticle`，末端生成 `PFMoonlord_SolarExplosion`。
...
- 绘制函数特效：`PreDraw` 使用 `ProvidenceMarkParticle` 和 `SoftRoundExplosion` 画圣火场。
- AI/命中粒子特效：主要是区域持续伤害，源码中没有额外粒子调用。

### PFProvidence_HolyRainOrb
- 绘制函数特效：`PreDraw` 手动画雨云/光球。
...
### PFRavager_Laser
- 绘制函数特效：`PreDraw` 使用 `BloomCircle`、`BloomLineSoftEdge`、`PearlParticleGlow`、`WaterFoam` 画血沸激光。
...
- 绘制函数特效：使用 `CalamityUtils.DrawAfterimagesCentered` 和 `LaserWallTelegraphBeam` 绘制 Artemis 激光/预警线。
- AI/命中粒子特效：主要是激光逻辑，源码中没有额外粒子调用。

### PFExoTwins_ApolloRocket
- 绘制函数特效：使用 `CalamityUtils.DrawAfterimagesCentered`，并叠 `ApolloRocketGlow`。
- AI/命中粒子特效：生成 Exo 主题尘埃，使用 ExoMechEffects 内部随机 dust type。

### PFExoAresLaserBeamStart
- 绘制函数特效：无常规 `PreDraw`；内部绘制使用 `AresLaserBeamMiddle` 与 `AresLaserBeamEnd` 分段激光贴图。
...
- 绘制函数特效：无常规 `PreDraw`；内部绘制使用 `ThanatosBeamMiddle` 与 `ThanatosBeamEnd` 分段激光贴图。
