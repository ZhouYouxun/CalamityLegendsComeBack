...
{
    public abstract class YC_WarshipBase : ModProjectile, ILocalizedModType, IYCRightBeamSource
    {
        ...
        public sealed override void AI()
        {
            ...
            if (!YC_WarshipHelper.TryGetHoldout(Projectile.owner, ParentHoldoutIndex, out Projectile holdoutProjectile, out YC_WarshipHoldout holdout))
            {
                Projectile.Kill();
                return;
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(texture, drawPosition, null, Color.White, Projectile.rotation, origin, Projectile.scale, SpriteEffects.None, 0);
            Main.EntitySpriteDraw(texture, drawPosition, null, OverlayColor, Projectile.rotation, origin, Projectile.scale * 1.08f, SpriteEffects.None, 0);
            return false;
        }

        protected abstract Vector2 GetLocalOffset();
        protected abstract float GetAngleOffsetDegrees();
        protected abstract void UpdateAttack(YC_WarshipHoldout holdout, Projectile holdoutProjectile);

        protected NPC FindTargetAhead(float range, float coneDegrees, bool requireLineOfSight = true)
        {
            return YC_WarshipHelper.FindTargetAhead(Projectile, Projectile.Center, CurrentForwardDirection, range, coneDegrees, requireLineOfSight);
        }
        ...
        protected void EmitMuzzleBurst(Vector2 direction, Color color, float speed, int dustCount)
        {
            for (int i = 0; i < dustCount; i++)
            {
                ...
                YC_WarshipHelper.EmitDust(Projectile.Center + direction * 10f, velocity, color, Main.rand.NextFloat(0.8f, 1.1f));
            }
        }

        private void UpdateStaticPosition(Projectile holdoutProjectile, YC_WarshipHoldout holdout)
        {
            ...
            Projectile.scale = ScaleBase + ScaleAmplitude * (float)System.Math.Sin(Main.GlobalTimeWrappedHourly * 4.5f + SlotIndex * 0.7f);
        }
