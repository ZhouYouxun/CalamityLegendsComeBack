...
{
    internal class BrimstoneFuryReBEAM : ModProjectile, ILocalizedModType
    {
        ...
        {
            BowChangeVFX.SpawnImpact(Projectile, BowChangeTheme.Brimstone, 0.8f);

            // 检查箭矢类型
            int ammoProjectile = (int)Projectile.ai[1]; // 从 ai[1] 获取弹药类型
            if (ammoProjectile == 0) // 如果没有指定类型，使用默认值
                ammoProjectile = ModContent.ProjectileType<BrimstoneFuryRePROJ>();

            // 沿激光路径生成弹幕
            float segmentLength = 130f; // 每段身体之间的间隔
            for (float i = 0; i < Projectile.ai[0]; i += segmentLength)
            {
                ...
                Projectile.NewProjectile(
                    ...
                    segmentPosition,
                    new Vector2(0, 10f), // 向下的初始速度
                    ammoProjectile, // 根据传递的弹药类型生成弹幕
                    Projectile.damage, // 使用激光的伤害
